// This file is an example how to create a test environment.
//
// Assume you want to test the ExampleClass. You create
// the class ExampleClassTest, which contains an instance
// of ExampleClass. This instance is used by all test functions,
// which is useful if setting up the class is more complex than
// just calling a constructor. You don't have to repeat this for
// each test.
//
// The test functions have the signature
// void testSomething(void);
// You just write them using the BOOST_CHECK, BOOST_CHECK_THROW etc.
// macros.
//
// Afterwards you register them with a test suite. The example below
// shows how to do it. As a final step you define init_unit_test_suite(),
// which is automatically called by boost and works like a main() function.
// It instantiates the test suite, which calls all registered test and
// keeps track of the bookkeeping how many tests failes or succeeded.
...
class ExampleClass {
public:
  static const int INITIAL_VALUE = 4711;
...
public:
  ExampleClassTestSuite() : test_suite("ExampleClass test suite") {
    ...
    // tests of non-static functions are added using BOOST_CLASS_TEST_CASE,
    // which needs a (shared) pointer to the instance
    add(BOOST_CLASS_TEST_CASE(&ExampleClassTest::testSomethingElse,
                              exampleClassTest));

    // in case of dependencies store the test cases before adding them and
    // declare the dependency
    test_case *writeTestCase =
        BOOST_CLASS_TEST_CASE(&ExampleClassTest::testWrite, exampleClassTest);
    test_case *readTestCase =
        BOOST_CLASS_TEST_CASE(&ExampleClassTest::testRead, exampleClassTest);

    // the write test uses read, so we first have to check that reading works
    // before we know that writing also does
...
void ExampleClassTest::testRead() {
  ...
  BOOST_CHECK(_exampleClass.read() == ExampleClass::INITIAL_VALUE);
}
...
void ExampleClassTest::testWrite() {
  ...
  // 3) check
  // we use read to check that the value was written. We checked before that
  // read acutally works.
